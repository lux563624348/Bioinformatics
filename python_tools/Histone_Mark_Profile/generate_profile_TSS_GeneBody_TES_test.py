...
### FUNCTION
def Get_Site_Profile(INPUT_read_file, INPUT_gtf_file, site_name, window_size, resolution, upstreamExtension,downstreamExtension):
    ...
    for alt in bedfile:
		alt = HTSeq.GenomicPosition(alt.iv.chrom, (alt.iv.start + alt.iv.end) / 2)
		ga[alt]+=1
		num_reads+=1
##########################################################################################
    ## Read gtf_file
...
def Get_GeneBody_Profile(INPUT_read_file, INPUT_gtf_file, genic_partition):
    ## Read Reads_File, and distribute them into GenomicArray.
    num_reads = 0
    ga = HTSeq.GenomicArray("auto", stranded=False, typecode="i")
    bedfile = HTSeq.BED_Reader(INPUT_read_file)
...
def Get_Bed_Region_Profile(INPUT_read_file, INPUT_bed_list, genic_partition):
    ## Read Reads_File, and distribute them into GenomicArray.
    ## Region
    import HTSeq
    num_reads = 0
    ...
    ### To calculate the number of transcripts that for profile plot.
    ### And also store position in a set.
    region_bedfile = HTSeq.BED_Reader(INPUT_bed_list)
...
def main(argv):
	...
	if (opt.type_site == 'GeneBody' ):
		###Up
		Upprofile = Get_Site_Profile(opt.bed_file, opt.gtf_file, 'TSS', opt.window_size,
		 opt.resolution, opt.upstreamExtension, opt.downstreamExtension)
		###Down
		Downprofile = Get_Site_Profile(opt.bed_file, opt.gtf_file, 'TES', opt.window_size,
		opt.resolution, opt.upstreamExtension, opt.downstreamExtension)
		### GENEBODY
		genebody_profile = Get_GeneBody_Profile(opt.bed_file, opt.gtf_file, opt.genic_partition)
		...
		norm_Site_profile = Get_Site_Profile(opt.bed_file, opt.gtf_file,  opt.type_site, opt.window_size,
		 opt.resolution, opt.upstreamExtension, opt.downstreamExtension)
	#### Second Site
