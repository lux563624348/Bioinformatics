...
def main(argv):
    ...
    parser.add_option("-r", "--'Promoter' or 'GeneBody' or 'PromoterGenebody' or 'ExonicRegion' or 'TSS'", action="store", type="string", dest="region_type", metavar="<str>", help="region")
    ...
    #if opt.promoter_upstream_extension:
    UpstreamExtension = opt.promoter_upstream_extension
    #if opt.promoter_downstream_extension:
    ...
    if opt.region_type == "Promoter":
        ...
        for feature in gtffile:
            if feature.type == "transcript_region" and feature.attr['gene_id'] not in gene_set:
                gene_set.add(feature.attr['gene_id'])
        ...
        gene_set = set()    
        for feature in gtffile:
            if feature.type == "three_UTR_region" and feature.attr['gene_id'] not in gene_set:
                gene_set.add(feature.attr['gene_id'])
