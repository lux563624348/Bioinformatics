...
def main(argv):
	...
	outline = "# GeneName" +  '\t' + "Read Count" + '\t' + "RPKM" + '\t' + "RPKM_pseudo_count" + '\n'
	f.write(outline)
	...
	for chrom in chroms:
		...
		if Utility_extended.fileExists(chrombed):
			...
			if len(gene_coords) > 0:
				...
				assert len(gene_name_list) == len (read_count_list)
				RPKM = [0] * len(gene_name_list)
				RPKM_pseudo = [0] * len(gene_name_list)
				for i in xrange(len(gene_name_list)):
					if region_length_list[i] > 0:
						RPKM[i] = read_count_list[i] / (region_length_list[i]/1000.0) / (totalcount/1000000.0)
						RPKM_pseudo[i] = (read_count_list[i] + 1) / (region_length_list[i]/1000.0) / (totalcount/1000000.0)
						outline = gene_name_list[i] + '\t' + str(read_count_list[i]) + '\t' + str(RPKM[i]) + '\t' + str(RPKM_pseudo[i]) + '\n'
						f.write(outline)
	f.close()
