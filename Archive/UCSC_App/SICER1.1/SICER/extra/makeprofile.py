...
def plot_data(x,read,fignum,range,title,xtitle,legend,outgraphname):
    pylab.figure(fignum)
...
def main(argv):
    ...
    plot_data(x_a,readp_a,0,opt.range,opt.title+' (+ reads)',opt.xtitle,opt.legend,opt.outfile+'_preads')
    plot_data(x_a,readm_a,1,opt.range,opt.title+' (- reads)',opt.xtitle,opt.legend,opt.outfile+'_nreads')
