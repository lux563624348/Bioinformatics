...
if [ $# -lt 10 ]; then
    echo ""
    echo 1>&2 Usage: $0 ["InputDir"] ["bed file"] ["control file"] ["OutputDir"] ["Species"] ["redundancy threshold"] ["fragment size"] ["effective genome fraction"]  ["gap size"] ["FDR"]  
    echo ""
    exit 1
fi
...
echo "python $SICER/src/calculate-window-size.py -s $SPECIES -b $OUTPUTDIR/$FILTEREDSAMPLEBED -f $FRAGMENT_SIZE"
let WINDOW_SIZE=`python $SICER/src/calculate-window-size.py -s $SPECIES -b $OUTPUTDIR/$FILTEREDSAMPLEBED -f $FRAGMENT_SIZE | grep Window | awk '{print $2}'`
let GAP_SIZE=$WINDOW_SIZE*$GAP_SIZE

echo " " 
echo "Our Window Size is: $WINDOW_SIZE and Our Gap size is: $GAP_SIZE"
...
#This file stores the significant islands in  "chr     start   end     ChIP_island_read_count" format
...
echo ""
echo ""
echo "Convert island summary to island bed file..."
...
echo "python $SICER/utility/filter_raw_tags_by_islands.py -s $SPECIES -a $SAMPLEDIR/$FILTEREDSAMPLEBED -i $FRAGMENT_SIZE -b $ISLANDS_BED_DIR/$ISLANDBED  -o  $ISLANDS_BED_DIR/$ISLANDFILTEREDRAWBED"
python $SICER/utility/filter_raw_tags_by_islands.py -s $SPECIES -a $ISLANDS_BED_DIR/$FILTEREDSAMPLEBED -i $FRAGMENT_SIZE -b $ISLANDS_BED_DIR/$ISLANDBED  -o  $ISLANDS_BED_DIR/$ISLANDFILTEREDRAWBED
