...
do
	...
	echo "IRTools expression -q IRI -i $INPUTDIR/${SAMPLE}.bam -p single -s fr-unstranded -e hg19 -u $MAPDIR/$MAPFILE -n $SAMPLE"
	IRTools expression -q IRI -i $INPUTDIR/${SAMPLE}.bam -p single -s fr-unstranded -e hg19 -u $MAPDIR/$MAPFILE -n $SAMPLE
	echo ""
	echo ""
