...
def main(argv):
	...
	target_name = "index8"
	control_name = "index6"
	summary_name = (opt.summary_name).split('/')[-1]
	summary_name = (summary_name).split('.')[0]
	
	print "\n\n", target_name, " vs ",  control_name
	# take care of the library size difference
	...
	# decode the pkl structure
	inf = open(opt.summary_name, 'rb')
	this_summary = pickle.load(inf)
	inf.close()
	print AnalyzeRNASeq.get_feature_names(this_summary)
	
	#myresult_age: {myid:(age, quality, myfc)}
	
	# Age vs derepression for individual REs
	rc_threshold = 3
	(myresult_age, myresult_age_on_mappable_REs) = AnalyzeRNASeq.age_vs_derepression_for_RE_family(this_summary, target_name, control_name, target_library_size_vs_control_library_size, rc_threshold, pc = 5)
	print "There are %d elements in %s " %(len(myresult_age.keys()), summary_name)
	...
	scatterplot(age, expression_fc, "age_vs_derepression_for_" + summary_name, xscale='linear', yscale='log')
	scatterplot(quality, expression_fc, "quality_vs_derepression_for_" + summary_name, xscale='linear', yscale='log')

	#myresult_age_on_mappable_REs: {myid:(age, quality, myfc)}
	print "There are %d %s elements that have reads above %d" %(len(myresult_age_on_mappable_REs.keys()), summary_name, rc_threshold)
	...
	scatterplot(age, expression_fc, "age_vs_derepression_for_mappable_" + summary_name, xscale='linear', yscale='log')
	#scatterplot(quality, expression_fc, "quality_vs_derepression_for_mappable_" + summary_name, xscale='linear', yscale='log')
	...
	family_names = AnalyzeRNASeq.find_family_name([this_summary])
	print "\n\nExamine the sandwich structure after RE clustering, ", family_names
	print "There are %d elements in %s" %(len(this_summary.keys()), family_names[0])
	...
	assembled_REs = AnalyzeRNASeq.explore_sandwich_structure_for_clusteredRE_family(this_summary, target_name, control_name, cluster_extension, extension, filtered_boundary_elements, target_library_size_vs_control_library_size, pc)
	
	RE_info = AnalyzeRNASeq.convert_assembled_RE_to_single_RE_annotation(assembled_REs, [this_summary], target_name, control_name, target_library_size_vs_control_library_size, pc)
	...
	scatterplot(num_of_boundary_elements, expression_fc_max, "comboRE_vs_derepression_for_clustered" + summary_name, xscale='linear', yscale='log')
	...
	path = "/home/data/mm9/Lin/processed/RepElements/brokendown/summary"
	os.chdir(path)
	summary_name_list = ["summary_on_LTR_ERV1_RLTR4_MM-int.pkl", "summary_on_LTR_ERV1_MuLV-int.pkl"]
	summaries = []
	for summary_file_name in summary_name_list:
	...
	assembled_REs = AnalyzeRNASeq.explore_sandwich_structure_for_clusteredRE_families(summaries, target_name, control_name, cluster_extension, extension, filtered_boundary_elements, target_library_size_vs_control_library_size, pc)
	os.chdir(current_dir)
	
	test_id = 'RLTR4_MM-intchr2-87360672'
	#print test_id
	#print summaries[0][test_id][target_name + "_rc"]
	#print summaries[0][test_id][control_name + "_rc"]
	print AnalyzeRNASeq.get_rc_for_RE_cluster(summaries, target_name, control_name,[test_id])
	
	AnalyzeRNASeq.output_assembled_REs(assembled_REs, target_name, control_name, "assembled_REs_summary.dat")
	
	RE_info = AnalyzeRNASeq.convert_assembled_RE_to_single_RE_annotation(assembled_REs, summaries, target_name, control_name, target_library_size_vs_control_library_size, pc)
	...
	fc_zero_LTR_assembled_REs =  AnalyzeRNASeq.get_fc_from_assembled_REs(zero_LTR_assembled_REs, target_name, control_name)
	...
	fc_Two_LTR_assembled_REs =  AnalyzeRNASeq.get_fc_from_assembled_REs(Two_LTR_assembled_REs, target_name, control_name)
	...
	plt.title(target_name + "_vs_" + control_name + "_on_assembled_REs, p-value = 2.7 E-77")
	plt.xlabel("log(Fold Change)")
	plt.ylabel("Frequency")
	plt.legend()
	#plt.legend(loc = 'upper left')
	plt.savefig(target_name + "_vs_" + control_name + "_on_assembled_REs" + "_logfc_hist" + ".eps", format="eps")
	plt.close()
	...
	plt.title(target_name + "_vs_" + control_name + "_on_assembled_REs, p-value = 5.4 E-44")
	#plt.xlabel('Month')
	#plt.ylabel('$log_{2} Age$')
	plt.ylabel('log(Divergence)')
	plt.ylim([0,10])
	plt.savefig(target_name + "_vs_" + control_name + "_on_assembled_REs" + "_logdivergence_boxplot" + ".eps", format="eps")
	plt.close()
	...
	print "\n\nFocus on 2_LTR REs"
	print "\n\nAll 2_LTR assembled_REs have read count above 20 in either %s or %s!" %(target_name, control_name)
	rc_threshold =10
	
	fc_threshold = 4
	Two_LTR_assembled_REs_derepressed, Two_LTR_assembled_REs_nonderepressed = AnalyzeRNASeq.classify_assembled_REs_by_fc(Two_LTR_assembled_REs, target_name, control_name, fc_threshold)
	...
	LTR_ages, myfc = AnalyzeRNASeq.correlate_two_features(Two_LTR_assembled_REs, "5_boundary_elements_age", "expression_fc_"+target_name+"_vs_" + control_name)
	...
	ages, myfc = AnalyzeRNASeq.correlate_two_features(Two_LTR_assembled_REs, "age", "expression_fc_"+target_name+"_vs_" + control_name)
