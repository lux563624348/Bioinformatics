...
echo "python $SICER/extra/calculate-window-size.py -s $SPECIES -b $DATADIR/$RAWBED -f $FRAGMENT_SIZE"
let WINDOW_SIZE=`python $SICER/src/calculate-window-size.py -s $SPECIES -b $DATADIR/$RAWBED -f $FRAGMENT_SIZE | grep Window | awk '{print $2}'`
