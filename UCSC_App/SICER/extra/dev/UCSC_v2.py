...
#To do: copy constructor, id system
...
class UCSC:
	"""
	Class for keeping known gene information.  
	...
	def get5UTR(self, upstream, downstream):
		...
		NOTICE:  if gene is on negative strand, the 5UTR start
		and end are still sequential
		"""
		include = 1
		if plus.match(self.strand):
	...
	def get3UTR(self, upstream, downstream):
		...
		If a gene is so short that the 3UTR end is beyond TSS, this gene will be discarded.
	
		NOTICE:  if gene is on negative strand, the 3UTR start
		and end are still sequential
		"""
		include = 1
		if plus.match(self.strand):
	...
	def getGeneEnd(self, upstream, downstream):
		...
		If a gene is so short that the promoter end is beyond TSS, this gene will be discarded.
...
class KnownGenes:
	...
	def IsInPromoter(self, chrom, position, strand, upstream, downstream):
		...
		If the position is not strand specific:
			returns the list of genes (ucsc objects) whose promoters encompass the position
		If the position is strand specific: 
			If the position is located in a promoter in the same strand, returns the genes (ucsc objects)
		...
		for chrom in self.gene_coords.keys():
			...
			for g in chrom_coords:
				if plus.match(g.strand):
					...
					# If no overlap with other genes, register
...
def getBoundariesMergedTranscripts(genes):
	# need to make sure they are on the same strand from outside
	starts = [gene.txStart for gene in genes]
	ends = [gene.txEnd for gene in genes]
	start = min(starts)
	end = max(ends)
	return [(start, end)]

def getMergedExonicRegions(genes):
	# Return the merged exons in the format of list of (start, end)
	all_exons = []
	for gene in genes:
		all_exons += gene.getExons()
	all_exons = sorted(all_exons, key = itemgetter(0))
	return Utility_extended.union(all_exons)


def getSharedExonicRegions(genes, min_width=5):
	"""
	shared_exons: a list of (start,end), which are shared among all genes in input, might breaking up existing exons.
	"""
	
	shared_exons = genes[0].getExons()
	for index in range(1, len(genes)):
		current_exons = genes[index].getExons()
		shared_exons = Utility_extended.intersect(shared_exons, current_exons, min_width)
	return shared_exons #sorted

def getInternalSharedExonicRegions(genes, min_width=5):
	"""
	This is used to avoid the interference of promoters and TES.
	"""
	shared_exons = getSharedExonicRegions(genes, min_width)
	if len(shared_exons) <= 2:
		return []
	else: 
		return shared_exons[1:-1]
		
def getSharedIntronicRegions(genes, min_width=5):
	"""
	shared_introns: a list of (start,end), which are shared among all genes in input, might breaking up existing introns.
	"""

	shared_introns = genes[0].getIntrons()
	for index in range(1, len(genes)):
		current_introns = genes[index].getIntrons()
...
def getShared3UTR(genes, min_width=5):
	"""
	shared_3UTRs: a list of (start,end), which are shared among all genes in input, might b.
	"""
	my_3UTRs=()
	for gene in genes:
		my_3UTR = genes.get3UTR(0,0)
		if len(my_3UTR) > 0:
