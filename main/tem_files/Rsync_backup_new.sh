...
TODAY=$(date +"%Y%m%d")
LASTWEEK=`date -d "7 day ago" +"%Y%m%d"`
LASTBACKUP="20191124"
BACKUP_LIFE=70
...
ssh -i ${PATH_SSH_Key} ${Destination_SSH} "mkdir -p ${Destination}"
...
OLDBACKUP=`date -d "${BACKUP_LIFE} days ago" +"%Y%m%d"`   # old backup has to be multiple of backup inteval. (in here is 7 days.)

ssh -i ${PATH_SSH_Key} ${Destination_SSH} "rm -R ${Destination_Path}/${OLDBACKUP}"
